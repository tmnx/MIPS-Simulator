 ...
 * 
 * Changed to compute up to 32 bits and type from int to long.
 *
 */
public class BitString {
	 ...
	 * decimal number 
	 * @param rsValue the unsigned value
	 */
	public void setValue(long rsValue) {
		// Check if it can be represented in the bits available
		if (rsValue < 0 || rsValue > MAX_UNSIGNED_VALUE) {
	 ...
	 * @param rsValue negative or positive decimal value
	 */
	public void setValue2sComp(long rsValue) {
		if (rsValue < MIN_VALUE || rsValue > MAX_VALUE) {
			throw new IllegalArgumentException("Cannot represent in "
					+ MAX_BITS + " bits.");
		}
		boolean isNegative = rsValue < 0;
		if (rsValue < 0) {
			rsValue *= -1;
		}
		setValue(rsValue);
